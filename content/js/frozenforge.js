...
var addTask = function (uid) { $('#loading-indicator-container').prepend('<div id="'+uid+'"></div>'); };
var endTask = function (uid) { $('#loading-indicator-container').children('#'+uid).remove(); };
...
function doPost(url, data, functionIfSuccessful) {
    ...
    $.post(url, data)
            ...
            .always(function() { endTask(uid); } );            
}

function loginUser(username, password) {
    $.post('ajax/LoginUserRequest', { USER_USERNAME : username, USER_PASSWORD : password } )
        .done(functionReceiveContent)
        .fail(functionConnectionFailed);
}
...
function logoutUser() {
    $.get('ajax/LogoutUserRequest', { } )
        .done(functionReceiveContent)
        .fail(functionConnectionFailed);
    return false;
}

function deleteUser(idOrUsername) {
    var userId      = isNaN(idOrUsername)? -1 : idOrUsername;
    var username    = isNaN(idOrUsername)? idOrUsername : '';
    $.get('ajax/DeleteUserRequest', { USER_ID : userId, USER_USERNAME : username } )
        .done(functionReceiveContent)
        .fail(functionConnectionFailed);
    return false;    
}
...
function toggleForm(target) {
    var $target = $(target);
    $target.slideToggle( function() { $(this).find('.has-error, .has-success, .has-warning, .form-control').val('').removeClass('has-error has-success has-warning'); });
}
...
$(document).on('blur', 'input,select', function() { validateField($(this)); });
